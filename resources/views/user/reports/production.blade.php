...
@section('content')
    ...
    <?php 
    $manual_reading = !empty($column_settings['columns']['manual_reading']) ? 1 : 0;
    ?>
    <!-- Page header start -->
    <div class="page-header card m-t-40">
        <div class="card-block caption-breadcrumb">
            <div class="breadcrumb-header">
                ...
                <hr>
            {{ Form::model($machine_search,array('route' => 'reports.production','id'=>'submit_report')) }}
            <div class="row">
                <div class="col-md-2">
                    <div class="form-group">
                        {{Form::label('report_types','Report Type')}}
                        {{Form::select('report_types',report_types(),null,['class'=>'form-control'])}}
                    </div>
                </div>
                @if(hasAccess('shift'))
            ...
            <div class="row">
                <div class="col-md-12">
                    ...
                    {{ Form::button('Export',['type'=>'submit','data-target'=>'_blank','target'=>'_blank','formaction'=>route('reports.production.export'),'class'=>'btn hor-grd btn-grd-warning btn-round search_report']) }}
                    <a href="{{ route('reports.production',['reset'=>1]) }}" class="btn hor-grd btn-grd-inverse btn-round hover-white">Reset</a>
                    @if($manual_reading)
                    <a href="{{ route('reading.production') }}" class="btn btn-danger btn-round reading">Manual Reading</a>
                    @endif
                </div>
            </div>
            {{ Form::close() }}
